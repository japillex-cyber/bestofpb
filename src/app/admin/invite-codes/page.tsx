...
export default function AdminInviteCodesPage() {
  ...
  return (
    <>
      <AdminLayout title="Invite Codes" subtitle="Manage who can register on Best of PB">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 24 }}>
          {[
            ...
            { label: 'Active', value: codes.filter(c => c.isActive).length, icon: '✅' },
            { label: 'Total Uses', value: codes.reduce((s, c) => s + c.usedCount, 0), icon: '👥' },
          ].map(s => (
            ...
            <tbody>
              {codes.map((code, i) => (
                <tr key={code.id} style={{ borderBottom: i < codes.length - 1 ? '1px solid var(--gray-100)' : 'none' }}>
                  ...
                  <td style={{ padding: '14px 16px', fontSize: 13, color: 'var(--gray-400)' }}>{code.expiresAt ?? 'Never'}</td>
                  <td style={{ padding: '14px 16px', fontSize: 13, color: 'var(--gray-500)' }}>{code.note}</td>
                  <td style={{ padding: '14px 16px' }}>
                    <button
      ...
      {/* Create modal — outside AdminLayout */}
      {showForm && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(4,9,28,0.8)', backdropFilter: 'blur(6px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
          onClick={(e) => { if (e.target === e.currentTarget) setShowForm(false) }}
        >
          <div style={{ background: '#fff', borderRadius: 20, padding: 32, width: '100%', maxWidth: 440, boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 20, fontWeight: 800 }}>Create Invite Code</h2>
              <button onClick={() => setShowForm(false)} style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--gray-100)', border: 'none', fontSize: 20, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit', lineHeight: 1 }}>×</button>
            </div>
            <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--gray-500)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 7 }}>Max Uses (blank = unlimited)</label>
              ...
              <button type="submit" disabled={loading} style={{ width: '100%', height: 48, background: loading ? '#9AA3B8' : '#0057FF', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer', marginTop: 4 }}>
