  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', background: '#04091C' }}>
        <img
          src="https://images.unsplash.com/photo-1534251369789-5067783b2dc7?w=1200&q=80"
          alt="Pacific Beach"
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 48 }}>
          ...
          <div>
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', marginBottom: 16, fontWeight: 500, letterSpacing: '0.3px' }}>Join 2,400+ members enjoying</p>
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {['Exclusive deals at 50+ local businesses','Curated PB experience packages','Community events & nominations','Your own digital membership card'].map(perk => (
                <li key={perk} style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 17, color: 'rgba(255,255,255,0.85)', paddingLeft: 24, position: 'relative', fontWeight: 500 }}>
          ...
          {step === 'invite' ? (
            <>
              ...
              <div style={{ marginBottom: 32 }}>
                ...
                <h1 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 30, fontWeight: 800, color: '#0D1020', letterSpacing: '-0.5px', marginBottom: 8 }}>Invite-only access</h1>
              ...
              <form onSubmit={verifyCode} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. BOPB-2024-LAUNCH"
                    value={inviteCode}
                    onChange={e => setInviteCode(e.target.value.toUpperCase())}
                    required
                    autoFocus
                    style={{ ...inputStyle, textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600 }}
                  />
                ...
                <button
                  ...
                  style={{ width: '100%', height: 50, background: inviteLoading ? '#9AA3B8' : '#0057FF', color: '#fff', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}
                >
              ...
              <div style={{ textAlign: 'center', marginTop: 20 }}>
                ...
                <p style={{ fontSize: 13, color: '#9AA3B8', marginTop: 8 }}>Already have an account? <Link href="/login" style={{ color: '#0057FF', fontWeight: 600 }}>Sign in</Link></p>
              </div>
            </>
          ) : (
            <>
              ...
              <div style={{ marginBottom: 28 }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#E8F5EE', color: '#0B7A4B', fontSize: 12, fontWeight: 600, padding: '5px 12px', borderRadius: 999, marginBottom: 16 }}>
                  ✓ Invite code verified
                </div>
                <h1 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 30, fontWeight: 800, color: '#0D1020', letterSpacing: '-0.5px', marginBottom: 8 }}>Create your account</h1>
                <p style={{ fontSize: 14, color: '#6B7590' }}>Already have an account? <Link href="/login" style={{ color: '#0057FF', fontWeight: 600 }}>Sign in</Link></p>
              </div>
              ...
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, width: '100%', height: 48, background: '#fff', border: '1.5px solid #E4E7EF', borderRadius: 10, fontSize: 14, fontWeight: 600, color: '#2E3448', fontFamily: 'inherit', cursor: 'pointer', marginBottom: 20 }}
              >
              ...
              <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div>
                    ...
                    <input type="password" placeholder="Min. 8 chars" value={password} onChange={e => setPassword(e.target.value)} required style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Confirm</label>
                    <input type="password" placeholder="Repeat" value={confirm} onChange={e => setConfirm(e.target.value)} required style={inputStyle} />
                  </div>
                </div>
                <button
                  type="submit"
                  disabled={loading}
                  style={{ width: '100%', height: 50, background: loading ? '#9AA3B8' : '#0057FF', color: '#fff', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer', marginTop: 4 }}
                >
